...
**Single-file frontend.** `App.jsx` holds all 23 screens. Inline `style={s.foo}` against a shared `s` map. Screen routing via `currentScreen` state + `SCREENS` enum, not React Router. Don't split casually. Two narrow exceptions are extracted modules: `groupAlerts.js` (testable in isolation, covered by vitest) and `copy.js` (centralized user-facing strings — single swap-point for tone, currently leans "friendly to new voters / immigrants" rather than the original accountability framing). `COPY` currently covers Dashboard, Alerts (federal + state), Events (list + detail), and Settings (issues editor block). The previous all-caps "terminal" tone is gone app-wide — Events `AI summary` / `Schedule` / `Committees` / `Witnesses` / `Legislation` / `News coverage`, `alerts.urgentBadge: "Urgent"`, and `events.offlineBadge: "Offline — sample data"` were sentence-cased in the whole-app warm pass alongside `s.sectionTitle` dropping `textTransform: uppercase`. New user-visible strings on those screens should still go through `COPY`; other screens (profiles, take-action, contact) are not yet wired.
