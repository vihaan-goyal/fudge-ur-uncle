...
- **Federal scheduled-vote ingester.** Federal `scheduled_votes` rows come exclusively from `seed.py` (5 hand-curated bills, all stamped on the day `python -m backend.alerts.seed` was run). There's no live counterpart to `ingest_state_votes.py` for the federal side, so the donation×vote matchmaking pool drifts toward zero as seed dates fall behind `today` — a typical run sees only the 1–2 future-dated rows still inside the 30-day lookahead. Congress.gov's `/v3/` API doesn't expose an upcoming-floor-vote endpoint; a real ingester would mirror the state pipeline's bill-status proxy (filter `congress_gov.search_bills` to bills awaiting floor action, project `scheduled_date = action_date + lead_days`). Refreshing `seed.py` dates is the quick demo workaround, but the federal alerts feed will stay degraded until a status-proxy ingester ships. State alerts are unaffected (their pipeline pulls from Legiscan masterlist independently).
