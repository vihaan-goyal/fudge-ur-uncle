...
- State (`ingest_state_votes.py`): Legiscan masterlist filtered to `STATUS_ENGROSSED`. **Title-only categorization** via `state_categories.py` keyword regex (13 categories: environment, healthcare, economy, defense, infrastructure, technology, labor, agriculture, housing, education, immigration, firearms, elections). Description fallback was removed — long policy summaries produced too many incidental matches. **AI fallback (`ai_categorize` in `state_categories.py`) runs only on regex misses** — gpt-4o-mini, title+description, single-word output constrained to the 13 categories or "none", cached 30d under `cat:{sha1(title+desc)}`. Result tracked in `stats["ai_recovered"]`; uncategorized titles still logged so the regex can grow over time. Behavior identical to pre-AI when `OPENAI_API_KEY` is unset. No per-bill `getBill` (would burn the 30k/mo quota). Masterlist cached 6h.
...
Test suite: 167 tests across `test_smoke.py` (HTTP shape, auth, alerts), `test_pipeline.py` (federal/state scoring + stale-sweep + state-calibration anchor at 0.554), `test_classifier.py` (PAC classifier word-boundary + transport-vs-union regressions), `test_state_categories.py` (regex + AI fallback wiring), `test_legiscan_vote_index.py` (per-state vote-index build + fast-path read), `test_promises_fallbacks.py` (JSON-LD/noscript extraction + ladder traversal).
